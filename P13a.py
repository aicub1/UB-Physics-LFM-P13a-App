...
class GUIapp(threading.Thread):
    ...
    def run(self):
        ...
        titT = tkinter.Label(self.root, text="T (°C)", bg=bgcolor, fg="black", font=textfont5b)
        ...
        titdT = tkinter.Label(self.root, text="δT (°C)", bg=bgcolor, fg="black", font=textfont5b)
...
errorT_formtext = ("{:^4s}".format("δ(T)   (°C)"))
...
while Taux >= TLIM:
 ...
 errorTemperaturaC = float(DadesPortSerieOUT[1])     # δ(T) (°C)
 ...
 AUXTemperatura.append(TemperaturaC)             # T
 AUXerrorTemperatura.append(errorTemperaturaC)   # δ(T)
 AUXIntensitat.append(Intensitat)                # I
 AUXerrorIntensitat.append(errorIntensitat)      # δ(I)
 AUXDifV.append(DifV)                            # dV
 AUXerrorDifV.append(errorDifV)                  # δ(dV)

 # Format de les dades
  
 T_format = ("{:5.2f}".format(TemperaturaC))
 errorT_format = ("{:3.2f}".format(errorTemperaturaC))
 ...
 app.s2.set("\nEn arribar la mostra de Ge a la seva temperatura màxima,\n"+
            "començarà l'enregistrament de dades:\n")
 ...
 if Temperatura < TLIM:
  ...
  app.s3green.set("\nL'ADQUISICIÓ DE DADES HA FINALITZAT CORRECTAMENT\n"+
             "\nPodeu sortir de l'aplicació\n")
...
if tempON == 1:
   ...
   TmaxAssolida = max(AUXTemperatura)
   app.sTmax.set(str("{:5.2f}".format(TmaxAssolida)))
